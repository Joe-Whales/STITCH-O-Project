...
class DVNet(nn.Module):
    ...
    def forward(self, x, normal_x):
        with torch.no_grad():
            features = self.feature_extractor(x)
            normal_features = self.feature_extractor(normal_x)
        reconstructed = self.unet(features)
        return reconstructed, normal_features

def train_model(model, train_loader, num_epochs, learning_rate, weight_decay, device, model_path, checkpoint=None, val_loader=None):
    model.to(device)
    ...
    for epoch in range(num_epochs):
        ...
        for i, input in progress_bar:
            normal_images = input["normal_image"].to(device)
            abnormal_images = input["abnormal_image"].to(device)
            
            optimizer.zero_grad()
            with autocast():
                reconstructed_features, normal_features = model(abnormal_images, normal_images)
                loss = criterion(reconstructed_features, normal_features.detach())
            
            scaler.scale(loss).backward()
            scaler.step(optimizer)
            scaler.update()
            
            train_loss += loss.item() * abnormal_images.size(0)
...
def validate(val_loader, model, verbose=False):
    ...
    with torch.no_grad():
        for i, input in enumerate(val_loader):
            ...
            normal_image = input["normal_image"].cuda()
            reconstructed_features, normal_features = model(image, normal_image)

            loss = criterion(reconstructed_features, normal_features).item()
            classes[clsname]["losses"].append(loss)
